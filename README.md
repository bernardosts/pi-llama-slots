...
|------|----------|-----------|-------|----------------------------|
| 1 | 281.3s | 324.5s | -43.2s | ~6k |
| 2 | 226.3s | 249.6s | -23.3s | ~17k |
| 3 | 254.2s | 332.3s | -78.1s | ~18k |
| 4 | 341.5s | 359.9s | -18.5s | ~19k |
| 5 | 237.0s | 398.0s | -161.0s | ~19k |
| 6 | 382.0s | 396.7s | -14.7s | ~20k |
| 7 | 337.1s | 392.6s | -55.6s | ~21k |
| 8 | 364.6s | 439.5s | -74.9s | ~22k |
| 9 | 313.2s | 440.2s | -127.0s | ~23k |
| 10 | 419.6s | 467.0s | -47.4s | ~24k |
...
2. **Magnitude varies with context:** The delta ranges from -14.7s (task 6, ~20k tokens) to -161.0s (task 5, ~19k tokens). There is no clear monotonic correlation with context size, suggesting other factors (model processing variability, KV cache fragmentation) also play a role. The average delta of 64.4s represents a **17% improvement**.

3. **High absolute prefill times:** Both ON and OFF show very high prefill times (226–467s). This is expected on the Intel Core Ultra 7 258V iGPU at ~24k tokens with unsloth/Qwen3.6-35B-A3B-MTP-GGUF @ UD-IQ4_XS — the encoding throughput (~15 t/s) means full re-encode of 24k tokens takes ~1600s theoretical, but actual observed times are lower due to partial re-encoding and hardware caching effects.
...
1. **Sequential execution only.** Subagent dispatches are strictly non-interleaved — each subagent completes before the next is dispatched. This is by design for the `-np 1` hardware constraint, but means the benchmark does not cover concurrent dispatch patterns.
2. **Model-driven task generation, not scripted replay.** Tasks are generated by the LLM reading the scenario file rather than being replayed from a fixed script. The ON and OFF runs are not guaranteed to execute byte-identical work — the prompts are the same, but the LLM may produce different tool call sequences or output lengths on each run.
3. **Cache-hit inference from wall-clock timing.** A shorter prefill gap is interpreted as a cache hit, but this is inferred from timing alone. No request-level KV cache telemetry (e.g., `cache_hit` flags from llama-server) is used to confirm cache state.
